...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="结果目录，默认 ./eval-runs（云端习惯 --out-dir /opt/datapilot-backups/eval-runs）",
    )
    args = ap.parse_args()
    ...
    print(
        f"平均耗时 {sum(r['elapsed'] for r in results) / len(results):.1f}s | "
    ...
    for r in results:
        ...
        print(
            f"  {'✅' if r['passed'] else '❌'} {r['case_id']} {r['score']:.2f} "
        ...
        json.dumps(
            {
                ...
                "abstain_correct": sum(
                    1 for r in results if r.get("expected_answer_type") == "abstain" and r["passed"]
                ),
