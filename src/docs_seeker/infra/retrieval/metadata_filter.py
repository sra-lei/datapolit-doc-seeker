"""docs-seeker - 问题中的结构化元数据解析与过滤表达式构造。
...
本模块只做三件纯函数的事（无 IO、可单测）：
1. ``parse_question_metadata``：从问题里解析「第X章/节/条/款/项」；
2. ``build_milvus_expr``：把解析结果变成 Milvus 过滤表达式（dense 路用）；
3. ``matches_metadata``：把解析结果变成 Python 谓词（BM25 进程内索引用）。
...
def build_milvus_expr(meta: dict[str, list[str]] | None) -> str:
    ...
    字段与取值都由本模块生成（字段取自常量表、取值只含数字与中文字符），
    不接受任意用户输入拼接，无注入面。
    """
    if not meta:
        return ""
