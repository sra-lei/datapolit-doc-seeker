...
LLM 调用链上（middleware 机制）。守卫规则与模式表在 ``base.py`` /
``builtin.py``，这里只负责「从 ``LLMRequest`` 提取待扫文本 → 调链 → 放行」。
...
from docs_seeker.llm.middleware.guards.base import LLM_MESSAGES_CTX, GuardChain
