...
依赖（ChatService / UsageTracker）全部由组合根注入。当前为单实例部署，
warmup_once 仅由本类唯一的后台线程调用，无需互斥；预热本身幂等（重复写同一
缓存 key），将来多副本部署确有需要时可再引入分布式锁。
"""
...
from docs_seeker.llm.middleware.guards import ANSWER_CTX
...
class TopQuestionWarmup:
    ...
    def __init__(self, service: ChatService, usage_tracker: UsageTracker):
